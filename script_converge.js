...
const ANGLES = [8 * Math.PI / 16]
const VIEWCOND = ['FIX', 'OVERT', 'COVERT'];
const VIEWTIME = [250]; // milliseconds
const PREPTIME = 3500; // milliseconds
const DELAY = [250]; // milliseconds
const TTC = 1000; // milliseconds
const TRANGE = 100; // milliseconds
const NTRIAL = 10;
...
function drawFrame(tAnim) {
    for (i = 0; i < NPLAY; i++) {
        el = document.getElementById('p' + i);
        el.style.left = DIM[0] / 2 + (play_dist[i] - (play_speed[i] / 1000) * tAnim) * Math.cos(i * obj_angle) + 'px';
        el.style.top = DIM[1] / 2 + (play_dist[i] - (play_speed[i] / 1000) * tAnim) * Math.sin(i * obj_angle) + "px";
    }
}
function initStim() {
    if (tPrep < PREPTIME) {
...
function startTrial() {
    ...
    if (blockNo < NTRIAL) {
        ...
        // Set TTC Delay
        ttc_delay = (DELAY[Math.floor(Math.random() * DELAY.length)]) / 1000; // seconds
        // Set TTC
        ttc_prime = (TTC + Math.random() * TRANGE) / 1000; // seconds
        console.log(ttc_prime)
        play_ttc = [ttc_prime, ttc_prime + (Math.round(Math.random()) * 2 - 1) * ttc_delay];
        // Set Distances
        play_dist = [PRAD + Math.random() * PRANGE, PRAD + Math.random() * PRANGE];
        // Set Speeds
        play_speed = [play_dist[0] / play_ttc[0], play_dist[1] / play_ttc[1]];
        // Set View Condition
